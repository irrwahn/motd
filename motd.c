 ...
 * 
 * This is basically an example for how things can get out of 
 * hand, once you start using defensive, robust coding while 
...
static void print_usage( const char *argv0 )
{
	...
	fprintf( stderr, 
		...
		"  -t : Set motto text file path; default: motd.txt.\n"
		"  -i : Set index file path; default: motd.idx.\n"
...
{
	int opt;
	const char *optstr = ":d:D:i:t:hr";
	const char *txt_path = "motd.txt";
	const char *idx_path = "motd.idx";
	wchar_t delim = L'÷';	/* division sign, unicode codepoint 247 */
	int regen_idx = 0;
	char *p;
	...
	srandom( time( NULL ) + clock() + getpid() );
	errno = 0;
